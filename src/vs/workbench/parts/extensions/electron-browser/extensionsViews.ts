...
export class ExtensionsListView extends ViewletPanel {
	...
	private async query(value: string): Promise<IPagedModel<IExtension>> {
		...
		value = value.replace(/\bcategory:("([^"]*)"|([^"]\S*))(\s+|\b|$)/g, (_, quotedCategory: string, unQuotedCategory: string) => {
			if (quotedCategory) {
				categories.push(unQuotedCategory.substr(1, unQuotedCategory.length - 2));
			} else {
				categories.push(unQuotedCategory);
			}
			return '';
		});
		...
		if (/@installed/i.test(value)) {
			...
			result = result
				.filter(e => e.type === LocalExtensionType.User
					...
					&& categories.every(category => e.local.manifest.categories.some(c => c === category)));
		...
		if (/@outdated/i.test(value)) {
			...
			const result = local
				...
				.filter(extension => extension.outdated
					...
					&& categories.every(category => extension.local.manifest.categories.some(c => c === category)));
